...
table = (
    ...
    , ( 'CMakeLists.txt'
        , r'set\W+optional_bare_version\W+"([0-9]+\.[0-9]+\.[0-9]+)"\W+$'
        , 'set( optional_bare_version "{major}.{minor}.{patch}" )\n' )

#    , ( 'example/cmake-pkg/CMakeLists.txt'
#        , r'set\W+optional_bare_version\W+"([0-9]+\.[0-9]+(\.[0-9]+)?)"\W+$'
#        , 'set( optional_bare_version "{major}.{minor}" )\n' )
#
...
#        , 'optional_bare_version = "{major}.{minor}.{patch}"\n' )
    ...
    , ( 'include/nonstd/optional.hpp'
        , r'\#define\s+optional_bare_MINOR\s+[0-9]+\s*$'
        , '#define optional_bare_MINOR  {minor}' )

    , ( 'include/nonstd/optional.hpp'
        , r'\#define\s+optional_bare_PATCH\s+[0-9]+\s*$'
        , '#define optional_bare_PATCH  {patch}\n' )
)
